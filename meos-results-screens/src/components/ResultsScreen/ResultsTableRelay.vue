		...
		>
			...
			>
				...
				>
					...
					<th
						v-if="results.cls.legCount < column.maxRadioCount"
						:colspan="(column.maxRadioCount - results.cls.legCount) * 3"
					/>
				</tr>
				...
				>
					...
					<template v-for="(n, i) in results.cls.legCount">
						...
						<flash-cell :display-value="result.legs[i].time" :watch-value="result.legs[i].time" :key="result.id + '-' + result.legs[i].runnerId + '-time'" class="col-radioTime" :style="{ width: colRadioTime + 'px' }"></flash-cell>

						<!-- if no radio punch then print no brackets-->
						<flash-cell v-if="result.legs[i].time == null" :display-value="null" :watch-value="result.legs[i].time" :key="result.id + '-' + result.legs[i].runnerId + '-rank'" class="col-radioRank" :style="{ width: colRadioRank + 'px' }"></flash-cell>
						<flash-cell v-else-if="result.legs[i].status == 1 || result.legs[i].status == 0" :display-value="'(' + result.legs[i].rank + ')'" :watch-value="result.legs[i].time" :key="result.id + '-' + result.legs[i].runnerId + '-rank'" class="col-radioRank" :style="{ width: colRadioRank + 'px' }"></flash-cell>
						<flash-cell v-else :display-value="statusToRank[result.status]" :watch-value="result.legs[i].time" :key="result.id + '-' + result.legs[i].runnerId + '-rank'" class="col-radioRank" :style="{ width: colRadioRank + 'px' }"></flash-cell>

						<flash-cell :display-value="result.legs[i].diff" :watch-value="result.legs[i].time" :key="result.id + '-' + result.legs[i].runnerId + '-diff'" class="col-radioDiff" :style="{ width: colRadioDiff + 'px' }"></flash-cell>
	...
	export default {
		...
		computed: {
			...
			pages() {
				...
				for (cls of resultsResponse.cmpResults) {
					tableWidth = tableBaseWidth + tableRadioWidth * cls.legCount
					results = null
				...
				for (const page of pages) {
					for (const column of page) {
						column.maxRadioCount = Math.max(0, ...column.classes.map(c => c.cls.legCount))
					}
				}
		...
		methods: {
			...
			async refreshResults () {
				this.resultsResponse = await meosResultsApi.getResultsScreenRelay()
			},
